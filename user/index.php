...
if(isset($_POST["username"]) && isset($_POST["password"]))
{
  ...
  $validate_user_information = mysqli_query($connection,"select * from `user` where `username` = '".mysqli_real_escape_string($connection,$username)."' AND `password` = '".mysqli_real_escape_string($connection,$encrypted_md5_password)."' AND `status`='Active'");
  ...
  if(mysqli_num_rows($validate_user_information) == 1) //Check if the information of the user are valid or not
  {
    ...
    $_SESSION['userid'] = $get_user_information['userid'];
